import prismadb from "@/prisma/prismadb";
import { NextResponse } from "next/server";
import jwt from "jsonwebtoken";
import bcrypt from "bcryptjs";

export async function createUser(data: {user_name: string, email: string, password: string, password_confirmation: string}) {
    try {
        const {user_name, email, password} = data;

        const existingUser = await prismadb.user.findUnique({
            where: {email}
        });

        if(existingUser){
            return new Response(
                JSON.stringify({
                    status: "failed",
                    message: "Email already in use.",
                    data: []
                }),
                {status: 401}
            )
        }

        const password_hashed = await bcrypt.hash(password, 10);

        const user_count = await prismadb.user.count();

        const role = user_count === 0 ? 'MASTER_ADMIN' : undefined;

        const new_user = prismadb.user.create({
            data: {
                user_name,
                email,
                password_hashed,
                ...(role && {role})
            },
        });

        const token = jwt.sign(
            {
                user_id: new_user.id,
                email: new_user.email,
                role: new_user.role
            },
            process.env.JWT_SECRET ?? "abc",
            {expiresIn: '1hr'}
        )

        return new Response(
            JSON.stringify({
                status: "success",
                message: "User created successfully",
                data: new_user,
                token: token
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function getAllUsers() {
    try {
        const users = await prismadb.user.findMany();

        return new Response(
            JSON.stringify({
                status: "success",
                message: "Users retrieved successfully",
                data: users
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function getUserDetail(id: string) {
    try {
        const user_detail = await prismadb.user.findUnique({
            where: {id}
        });

        return new Response(
            JSON.stringify({
                status: "success",
                message: "Users detail retrieved successfully",
                data: user_detail
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function updateUserDetail(id: string, data: {id: string, user_name: string}) {
    const {user_name} = data;

    try {
        const updated_User = await prismadb.user.update({
            where: {id},
            data: {
                user_name
            }
        });

        return new Response(
            JSON.stringify({
                status: "success",
                message: "User updated successfully",
                data: updated_User
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function deleteUserDetail(id: string) {
    try {
        const deleted_User = await prismadb.user.delete({
            where: {id}
        })

        return new Response(
            JSON.stringify({
                status: "success",
                message: "User deleted successfully",
                data: deleted_User
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function loginUser(data: {email: string, password: string}) {
    try {
        const {email, password} = data;

        const existingUser = await prismadb.findUnique({
            where: {email}
        });

        if(!existingUser){
            return new Response(
                JSON.stringify({
                    status: "failed",
                    message: "Email doesn't exist.",
                    data: []
                }),
                {status: 400}
            )
        }

        const password_match = await bcrypt.compare(existingUser.password_hashed, password);

        if(!password_match){
            return new Response(
                JSON.stringify({
                    status: "failed",
                    message: "Wrong email or password.",
                    data: []
                }),
                {status: 400}
            )
        }

        const token = jwt.sign(
            {
                user_id: existingUser.id,
                email: existingUser.email,
                role: existingUser.role
            },
            process.env.JWT_SECRET ?? "abc",
            {expiresIn: '1hr'}
        )

        return new Response(
            JSON.stringify({
                status: "success",
                message: "User logged in successfully",
                data: existingUser,
                token: token
            }),
            { status: 200 }
        );
    }
    catch(error) {
        return new Response("Internal server error", {status: 500});
    }
}

export async function logoutUser() {
    try {

    }
    catch(error) {
        return new NextResponse("Internal server error", {status: 500});
    }
}